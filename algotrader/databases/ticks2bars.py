...
def one_minute_adjustment(datetime_to_adjust):
    ...
    # remove milliseconds as string, replace works with microseconds no milliseconds
...
def insert_bars_to_sec_master(client, bars, table_prefix, frequency, symbol, provider):
    ...
    try:
        ...
        logger.info('Insert successful for {} at {}'.format(symbol, table_name))
    except (InfluxDBClientError, InfluxDBServerError):
        logger.exception('Error inserting data for {} at {}'.format(tags, table_name))
        sys.exit(-1)


@fn_timer
def tick_resampling(symbol, provider, input_table, output_table_prefix, custom_dates=False,
    ...
    while start_time <= end_time:
        ...
        try:
            ...
            insert_bars_to_sec_master(client, bars, output_table_prefix, frequency, symbol, provider)
